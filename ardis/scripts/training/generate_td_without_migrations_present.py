'''
    ...
    To get the IPC and Energy from the current epoch, assuming the characteristics of the applications don't change significantly from one epoch to the next.
'''
...
def process_experiment_results(results_folder):
    dataset = []
    for frequency_folder in os.listdir(results_folder):
        frequency = frequency_folder
        #if "1800MHz" in frequency:
        path = os.path.join(results_folder, frequency_folder)
        for experiment_folder in os.listdir(path):
            ...
            if os.path.isfile(log_path):
                ...
                dataset.extend(create_dataset(epochs, frequency, log_path).to_dict(orient='records'))
    return pd.DataFrame(dataset)

# Example usage:
results_df = process_experiment_results(MOTIVATIONAL_RESULTS_FOLDER)
results_df.to_csv('present_contention.csv', index=False)
