...
class ThreadMappingClip(ResultClip):
    ...
    def create_plot(self, result_wrapper: ExperimentResultWrapper) -> Figure:
        ...
        for ax, iid in zip(axs, instances):
            ...
            relevant_threads = {tid: ranges for tid, ranges in thread_to_affinity_ranges.items() if tid in threads}
    ...
    def _plot_thread_mapping(
        ...
        max_threads_per_core = max(
            sum(1 for ranges in thread_to_affinity_ranges.values() if core in ranges)
            for core in utilized_cores
        )
        bar_height = 2 / max_threads_per_core if max_threads_per_core > 0 else 1
